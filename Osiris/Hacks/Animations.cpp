        ...
        if (player.simulationTime != entity->simulationTime() && player.simulationTime < entity->simulationTime())
        {
            ...
            else
            {
                ...
                for (int i = 1; i <= player.chokedPackets + 1; i++)
                {
                    ...
                    // Set the current time to the simulated time.
                    memory->globalVars->currenttime = simulatedTime;

                    // Clear the entity's flags.
                    entity->getEFlags() &= ~0x1000;

                    // Calculate the entity's new velocity and duck amount.
                    ...
                    // Check if the animation state's last update frame and time match the global variables.
                    // If they do, subtract 1 from the last update frame and add 1 tick to the last update time.
                    if (entity->getAnimstate()->lastUpdateFrame == memory->globalVars->framecount)
                        entity->getAnimstate()->lastUpdateFrame -= 1;
                    ...
                    // Update the entity's animation state.
                    entity->updateClientSideAnimation();
                ...
                // Set the entity's velocity and duck amount to the player's values.
                entity->getAbsVelocity() = player.velocity;
